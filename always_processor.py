...
def process_always_order(df, sh, spread):
    ...
    try:
        ...
        order_data = pd.DataFrame({
            ...
            '주문 id': df['주문아이디'].fillna('').astype(str),
            '주문 날짜': df['주문 시점'].fillna('').astype(str),
            '결제 날짜': df['주문 시점'].fillna('').astype(str),
            '판매금액': df['상품가격'].fillna('').astype(str),
            '할인금액': df['total_discount'].fillna('').astype(str),
            '플랫폼 비용': str(0),
            '정산금액': df['정산대상금액(수수료 제외)'].fillna('').astype(str),
