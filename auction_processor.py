...
def process_auction_order(df, sh, spread):
    ...
    try:
        ...
        order_data = pd.DataFrame({
            ...
            '주문 날짜': df['주문일자(결제확인전)'].fillna('').astype(str),
            '결제 날짜': df['결제일'].fillna('').astype(str),
            '판매금액': df['판매금액'].fillna('').astype(str),
            '할인금액': df['total_discount'].fillna('').astype(str),
            '플랫폼 비용': df['서비스이용료'].fillna('').astype(str),
            '정산금액': df['정산예정금액'].fillna('').astype(str),
            '배송비': df['배송비 금액'].fillna('').astype(str),
            '주문 수량': df['수량'].fillna('').astype(str),
            '사은품': df['사은품'].fillna('').astype(str),
            '주문 총 무게': '',
            '주문 상태': '',
            '플랫폼': '옥션',
            '기록날짜': pd.to_datetime('now').strftime('%Y-%m-%d %H:%M:%S')
