...
def process_eleven_order(df, sh, spread):
    ...
    try:
        ...
        order_data = pd.DataFrame({
            ...
            '주문 날짜': df['주문일시'].fillna('').astype(str),
            '결제 날짜': df['결제일시'].fillna('').astype(str),
            '판매금액': df['주문금액'].fillna('').astype(str),
            '할인금액': df['total_discount'].fillna('').astype(str),
            '플랫폼 비용': df['서비스이용료'].fillna('').astype(str),
            '정산금액': df['정산예정금액'].fillna('').astype(str),
            '배송비': df['배송비'].fillna('').astype(str),
            '주문 수량': df['수량'].fillna('').astype(str),
            '사은품': '',
            '주문 총 무게': '',
            '주문 상태': '',
            '플랫폼': '11st',
...
def process_eleven_delivery(df, sh, spread):
    ...
    try:
        ...
        delivery_data = pd.DataFrame({
            '배송 key': df['주문번호'].fillna('').astype(str).apply(lambda x: f"배송_{x}_11st"),
            '주문 key': df['주문번호'].fillna('').astype(str).apply(lambda x: f"{x}_11st"),
            '배송 주소': df['주소'].fillna('').astype(str),
            '배송 우편번호': df['우편번호'].fillna('').astype(str),
            '배송 메시지': df['배송메시지'].fillna('').astype(str),
            '출고 날짜': get_delivery_date(),
            '해당 배송회차': '1',
            '방문수령 여부': '',
            '방문수령 날짜': '',
            '수취자 휴대폰': df['휴대폰번호'].fillna('').astype(str),
            '수취자 전화번호': df['전화번호'].fillna('').astype(str),
            '수취자 이름': df['수취인'].fillna('').astype(str),
            '선착불 여부': '',
            '선착불 금액': '',
            '기록날짜': pd.to_datetime('now').strftime('%Y-%m-%d %H:%M:%S')
        })
